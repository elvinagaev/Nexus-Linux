...
def _find_repo_root(start: Path) -> Path | None:
    """Walk upward from `start` until the Nexus Linux repository root is found."""
    current = start
    for _ in range(10):
...
_SRC_ROOT = _REPO_ROOT / "nexus-installer" / "src" if _REPO_ROOT is not None else None

for _path in (_SHARED_ROOT, _SRC_ROOT):
    if _path is not None and str(_path) not in sys.path:
        sys.path.append(str(_path))

from .main_window import NexusInstallerWindow  # noqa: E402

__all__ = ["NexusInstallerWindow"]
