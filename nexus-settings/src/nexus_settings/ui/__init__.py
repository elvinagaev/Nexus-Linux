...
_SRC_ROOT = _REPO_ROOT / "nexus-settings" / "src" if _REPO_ROOT is not None else None

for _path in (_SHARED_ROOT, _SRC_ROOT):
    if _path is not None and str(_path) not in sys.path:
        sys.path.append(str(_path))

from .main_window import NexusSettingsWindow  # noqa: E402

__all__ = ["NexusSettingsWindow"]
